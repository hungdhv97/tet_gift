...
const Map: React.FC = () => {
    ...
    useEffect(() => {
        if (positionHistory) {
            const groupedMessages = positionHistory.reduce<Record<string, MessageResponse[]>>((acc, message) => {
                if (!acc[message.id]) {
                    acc[message.id] = [];
                }
    ...
    useEffect(() => {
        ...
        socket.onmessage = (event) => {
            ...
            setMessages((prevMessages) => {
                ...
                updatedMessages[parsedData.message.id].push(parsedData.message);
    ...
    return (
        <MapContainer center={[15.0, 110.0]} zoom={6} style={{ height: "100vh", width: "100%" }}>
            ...
            />
            {Object.values(messages).map((vesselPath) => {
                ...
                return (
                    <React.Fragment key={latestPosition.registration_number}>
